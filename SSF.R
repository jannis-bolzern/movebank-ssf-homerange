...
class_labels <- c(
  ...
  "40" = "Cropland", "50" = "Built_up", "60" = "Bare_sparse_vegetation",
  "70" = "Snow_and_ice", "80" = "Permanent_water_bodies",
  "90" = "Herbaceous_wetland", "95" = "Mangroves", "100" = "Moss_and_lichen"
)
...
ssf_coyote_struc <- glmmTMB(
  case_binary ~ -1 + 
    land_use * human_footprint +              # full interaction
    offset(log_sl_) +
    (1 | step_id_) +
    (0 + land_use + human_footprint || animal_id),
  family = poisson,
  data = coyote_final,
...
ssf_coyote_struc$mapArg = list(theta=factor(c(NA,1:9)))

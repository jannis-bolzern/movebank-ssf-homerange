...
make_track_map <- function(lines_sf,
  ...
  # Transform to Web Mercator (3857) for plotting
  lines_3857 <- st_transform(lines_sf, 3857)
  
  # If bbox not provided, create from data + buffer
  if (is.null(bbox)) {
    buf <- st_buffer(lines_sf, buffer_dist)
    bbox <- st_bbox(buf)
  } else {
    # Convert numeric bbox to sfc object
    bbox <- st_bbox(setNames(bbox, c("xmin", "ymin", "xmax", "ymax")), crs = 4326)
  }
  
  # Convert bbox to 3857 for tile fetching
  ...
  gg <- ggplot() +
    ...
    coord_sf(xlim = c(bbox_merc["xmin"], bbox_merc["xmax"]),
             ...
             expand = FALSE) +
    labs(title = map_title, caption = tile_provider) +
    theme(
...
ggplot(month_count, aes(x = as.factor(month_name), y = count, color = source)) +
  ...
  scale_x_discrete() +
  facet_wrap(~ ifelse(source %in% c("Bylot", "Herschel"), "Bylot/Herschel", "Wiltshire"), scales = "free_y") +
  labs(x = "Month", y = "Number of data points", fill = "Source", color = "Source") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90)) +
  theme(axis.text.x = element_text(size = 6)) +
  theme(axis.text.x = element_text(margin = margin(b = 20))) +
...
ggplot(bylot_mcp, aes(fill = as.factor(individual_id))) +
  geom_sf(alpha = 0.4) +
...
ggplot(wiltshire_mcp, aes(fill = as.factor(individual_id))) +
  geom_sf(alpha = 0.4) +
  labs(x = "Longitude", y = "Latitude", fill = "Fox id")

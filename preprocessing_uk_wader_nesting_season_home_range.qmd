...
ggsave("img/home_range_bylot_wader.png", width = 10, height = 6, dpi = 300)
```
...
ggplot() +
  ...
  geom_sf_text(data = wiltshire_centroids,
               aes(label = paste0(round(area_m2, 1), " km²")),
               color = "white", size = 3) +
...
wiltshire_area <- st_area(wiltshire_mcp) |> as.numeric() / 1e6

# Create dataframe
home_range_df <- data.frame(
  Area_km2 = c(bylot_area, wiltshire_area),
  Location = c(rep("Bylot", length(bylot_area)),
               rep("Wiltshire (24h)", length(wiltshire_area)))
)

# Identify extreme outlier (largest Wiltshire area)
outlier_value <- max(home_range_df$Area_km2[home_range_df$Location == "Wiltshire (24h)"])
outlier_index <- which(home_range_df$Area_km2 == outlier_value)

# Plot
ggplot(home_range_df, aes(x = Location, y = Area_km2, color = Location, fill = Location)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.2, linewidth = 1.2) +
  geom_point(shape = 21, size = 3, fill = "white", color = "black", stroke = 0.8, 
             position = position_jitter(width = 0.3, height = 0)) +
  geom_text(data = home_range_df[outlier_index, ],
            aes(label = paste0(round(Area_km2, 1), " km²")),
            vjust = 1.5, color = "white", size = 3.5) +
  scale_color_manual(values = c("Bylot" = "#DB4743FF", "Wiltshire (24h)" = "#5495CFFF")) +
  scale_fill_manual(values = c("Bylot" = "#DB4743FF", "Wiltshire (24h)" = "#5495CFFF")) +
  labs(
    x = NULL,
...
ggplot(map_data, aes(x = lon, y = lat)) +
  ...
  coord_fixed() +
  labs(x = "Longitude", y = "Latitude", size = "Median Area (km²)") +
  theme_minimal(base_size = 14) +
  theme(
    text = element_text(color = "white"),
    axis.text = element_text(color = "white"),

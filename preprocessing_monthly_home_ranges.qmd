...
calculate_monthly_home_ranges <- function(ts_start, num_months, study_id, license_string_with_quotes, epsg) {
  areas <- list()
  gg <- ggplot()
  ...
  for (i in 1:num_months) {
    ...
    tryCatch({
      ...
      # TBD: Should the transform happen here?
      tracks <- mt_track_lines(foxes)
      mcp <- st_convex_hull(tracks)
      if (epsg > 0) {
        # TBD: Didn't change anything, is this even executed?
        st_transform(mcp, epsg) 
      }
      areas[[i]] <- st_area(mcp)
...
```{r}
results_bylot <- calculate_monthly_home_ranges(as.POSIXct("2011-07-01 00:00:00"), 44, 1371322147, '9559cb1b1cca51e9f3fd69812e5e22dc', 0)
...
results_herschel = calculate_monthly_home_ranges(as.POSIXct("2009-07-01 00:00:00"), 9, 1371529289, '436e8205d0c2973115e2024b146a6ad5', 3154)
...
```{r}
results_wiltshire = calculate_monthly_home_ranges(as.POSIXct("2016-01-01 00:00:00"), 43, 3179890710, 'ffa36f79defe10eac9fe70f6502a17e0', 0)

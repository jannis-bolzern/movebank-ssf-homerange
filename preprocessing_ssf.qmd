...
## Fit SSF
```{r}
# Load data from file, or if model if data doesn't exist yet on file.
...
avg_eff_plot_hfp_landuse <- function(prediction, nsim = 100, k = 10, showPeakValue = TRUE) {
  ...
  fit_sample_matrix <- matrix(fit_sample, nrow = nrow(prediction), ncol = nsim)
  smooth_list <- list()
  for (j in 1:nsim) {
  ...
  p <- ggplot(avg_smooth, aes(x = hfp, y = est)) +
    ...
    facet_wrap(~land_use_grouped, scales = "fixed") +
    theme_minimal() +
...
# Overview plot, commented out because it takes forever to run and save.
# Note that there seems some kind of caching of commands until the data is
# actually required, therefore execute the ggsave to trigger execution.
...
#  theme_minimal()
#ggsave("img/coyote_effects_overview.png")

# Main plot. There are warnings but they are irrelevant.
avg_eff_output <- avg_eff_plot_hfp_landuse(coyote_ssf_pred, nsim = 100, k = 10, showPeakValue = TRUE)

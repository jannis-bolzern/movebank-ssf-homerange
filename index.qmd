...
The home range for the sub-sampled Wiltshire data is shown in @fig-home-ranges-wiltshire-sampled24h. It is 0.56 km<sup>2</sup> for the sampled data, which is roughly half as much as for the full data, which shows that the Wiltshire fox home ranges are overestimated in comparison to Bylot fox home ranges because of the higher amount of data points available. This demonstrates that the influence of sampling intervals is definitely present. However, it is secondary in comparison to the difference in fox behavior. 
...
For the purpose of home range comparison, two models were defined comprised of data, home range calculation based on minimum convex hulls, and median selection. The validation criteria required that the amount of difference between the results is significantly larger (i.e., more than 10 times) than the effects on the results introduced by data properties.

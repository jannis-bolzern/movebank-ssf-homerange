...
The Bylot/Herschel Canadian data was collected all year round, at a much lower sampling rate of once per day, at random different afternoon times of the day. The collection period was June 2009 to Feb 2010 for Herschel and from 2011 to 2015 for Bylot for two foxes per island. @fig-datapoints-year provides an overview of the amount of data points available per year, with more data from Wiltshire because of the higher number of foxes and the higher sampling rate.
...
As discussed in @sec-red-fox-data the data for the two locations have different temporal scale. @laube2011 have found that the choice of temporal scale has considerable effects on movement parameter calculations, in turn affecting home range results.
...
For #1, a possible approach to achieve similar sampling intervals would be to sample a random afternoon data point for each 24 hour window. However, this would include the implicit assumption that foxes will follow similar daily patterns in the different environments. For #2, a possible approach would be to compare the data for the same time of the year. However, the geographical location are different. The weather conditions will differ for the same day of the year, likely leading to different animal behavior. For #3, aggregated comparisons can solve the issue assuming there is enough data for the smaller data source.
...
The Herschel data is not applicable for this approach since it has minimal overlaps with the Wiltshire data (see @fig-datapoints-month). To assess the impact of sampling intervals for problem #1, the Wiltshire data was downsampled. A random data point from every 24 hour period was selected. 

**TBD:** Decide if we should compare median monthly home range per animal and location
...
## Results
The resulting home ranges are shown in @fig-home-ranges-bylot and @fig-home-ranges-wiltshire. The median home rage size for the foxes in Herschel (75.3 km<sup>2</sup>) is more than 50 times bigger compared to the foxes in Wiltshire (1.1 km<sup>2</sup>). 
...
The fox home range results differ enormously between rural and remote areas. We can conclude that human presence massively impacts fox movement behavior, with availability of food likely being relevant, but we have not been able to single out that factor.

**TBD:** Should we change the research question 1 to "human activity" instead of "access to food sources"?

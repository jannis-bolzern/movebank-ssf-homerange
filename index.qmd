...
This section describes the dataset, the steps taken to prepare and process the different data set in use, and the methodological approach that was employed.
...
#### Movebank
We have used data from the Movebank database and system by @kays2022movebank. All the datasets in Movebank have the same format and table definitions, which simplifies data handling since it allows to re-use R code between datasets. A library for data ingestion (see [@fig-code-chunk] for an example) and trajectory handling in R is provided by @bartk_move2.
...
The main challenge for this research question is the fact that data from different sources is compared. Upon closer inspection it turns out that even if the datasets have the exact same format, the data itself shows completely different properties across the two sources Wiltshire and Bylot/Herschel.
...
The Bylot/Herschel Canadian data was collected all year round, but at a much lower sampling rate of just once per day. The collection period was from 2011 to 2015 for Bylot and from June 2009 to Feb 2010 for Herschel, at randomly different afternoon times of the day. It included a much lower number of foxes, two per island. @fig-datapoints-year provides an overview of the amount of data points available per year, demonstrating the fact that there is much more data from Wiltshire because of the higher number of foxes and also the higher sampling rate.
...
We need to make sure that the datasets cover comparable time ranges. We can decide to compare the data for the same time of the year. @fig-datapoints-month shows that the Herschel data can not be used for this because it does only minimally overlap with the UK wader nesting season.
...
@fig-datapoints-month also reveals there is much less data from Bylot for the time period of the UK wader nesting season compared to other times of the year. It could be that the foxes have different yearly activity patterns on the two continent because of the seasonal differences between the locations.
...
@laube2011 have found that the choice of temporal scale has considerable effects on the outcome of movement parameter calculations. Speed, sinuosity and turning angle of animal trajectories will differ based on the sampling interval. Based on these insights we suspect that temporal scale will also affect home range results. Therefore, we need to ensure that the sampling intervals are similar between the datasets for an apples-to-apples comparison. The simplest acceptable solution is to select a random data point from every 24 hour period for the Wiltshire data. This would not be possible for many use cases except for home range calculation.
...
foxes_bylot_monthly <- foxes_bylot %>% mt_set_track_id(paste(mt_track_id(.),
    sep = "_", month.name[month(mt_time(.))]
  ))
```

```{r}
foxes_bylot_monthly_filtered_may <- foxes_bylot_monthly |>
  #filter_track_data(individual_local_identifier == "JBJB_May") # worked for Herschel
  filter_track_data(deployment_id == "1371336385_May")
```


#### HRP-100

Use 2020 data and make a subset as quickly as possible since the dataset is very big.
...
w_tracks <- mt_track_lines(foxes_wiltshire_sampled_24h)
...
c1_tracks <- mt_track_lines(foxes_bylot_monthly_filtered_may)

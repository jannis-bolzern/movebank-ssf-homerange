...
@laube2011 have found that the temporal scale affects the calculation of movement parameters. The speed, sinuosity and turning angle of animal trajectories will differ based on the sampling interval. We conclude that temporal scale will also affect home range results. Therefore we need to check if the sampling intervals are similar between the datasets to ensure an apples-to-apples comparison.

The Wiltshire data is collected at 10 or 60 minute intervals. The data collection team had the option to select the sampling interval remotely. The Canadian data is collected only once per day

We also need to make sure that they cover similar time ranges.

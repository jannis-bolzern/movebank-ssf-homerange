...
To compare land cover choices, which translate into habitat characteristics, several statistical approaches are possible. The classical approach is to use conditional logistic regression to compare between observed and randim steps (clogit, see @manly2007resource). However, for data with several animals the approach by @muff2020accounting is better suited.

Ich habe mich ausserdem intensiv mit der Methode von Muff et al. auseinandergesetzt: Die Idee ist, dass die conditional logistic regression (wie sie bei survival::clogit verwendet wird) likelihood-äquivalent zu einem Poisson-Modell mit stratum-spezifischen Fixinterzepten ist. 

Wenn man diese Interzepte dann als Random Effects mit einer sehr grossen festen Varianz interpretiert, kann man die ganze SSF als GLMM formulieren – und das ermöglicht uns auch, Random Slopes pro Individuum einzubauen.
